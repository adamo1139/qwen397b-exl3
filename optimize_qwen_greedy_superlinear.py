...
The optimal assignment has clean bands — highest-KLD get 8bpw, then 5bpw, 4bpw, 3bpw.

Sweeps n8 and n5 to meet the bit budget, picks split with minimum total error.

Fixed components (attention, norms, embeddings, etc.) hardcoded at 8bpw.

SUPERLINEAR PENALTY: Uses KLD^1.5 to account for non-linear quantization error.
...
#   8bpw=0.00255, 5bpw=0.0079, 4bpw=0.0203, 3bpw=0.0674, 2bpw=0.516
# Superlinear penalty: raise KLD ratios to power 1.5 to model non-linear quant difficulty
KLD = {8: 0.00255, 5: 0.0079, 4: 0.0203, 3: 0.0674, 2: 0.516}
...
def optimize(target_avg: float, candidates: list[tuple[int, str, float]]):
    """
    Find optimal five-band split: 8 | 5 | 4 | 3 | 2 bpw.

    Sweep n8 and n5, then solve for n4, n3, n2 from remaining budget.
    Assignments: top n8→8bpw, next n5→5bpw, next n4→4bpw, next n3→3bpw, rest n2→2bpw.
    """
    N = len(candidates)
    ...
    max_n8 = int(target_bits / 8)

    for n8 in range(0, max_n8 + 1):
        after8_bits = target_bits - 8 * n8
        after8_cnt = N - n8
        if after8_bits < 2 * after8_cnt or after8_bits > 5 * after8_cnt:
            continue

        max_n5 = min(after8_cnt, int(after8_bits / 5))
        for n5 in range(0, max_n5 + 1):
            # remaining for {4,3,2} bands
            R = after8_cnt - n5
            Rb = int(round(after8_bits - 5 * n5))
            if Rb < 2 * R or Rb > 4 * R:
                continue
            ...
            for n4 in range(n4_min, n4_max + 1):
                ...
                actual = 8 * n8 + 5 * n5 + 4 * n4 + 3 * n3 + 2 * n2
                if abs(actual / N - target_avg) > 0.005:
                    continue

                # Assign: top n8→8, n5→5, n4→4, n3→3, n2→2
                cost = 0.0
                assignments = {}
                i = 0
...
def generate_yaml(assignments: dict[int, dict[str, int]], fixed_layers: dict, layer_range: list[int]) -> str:
    ...
    lines = [
        "# Optimized using KLD sensitivity (greedy sort-and-assign, 8bpw capable)",
        "# Model: Qwen3.5-397B-A17B MoE",
        "",
        "sources:",
        "  - id: 2",
        "    model_dir: /.../models/Qwen3.5-397B-A17B-exl3-2bpw",
        "  - id: 3",
        "    model_dir: /.../Qwen3.5-397B-A17B-exl3-3bpw",
        "  - id: 4",
        "    model_dir: /.../models/Qwen3.5-397B-A17B-exl3-4bpw",
        "  - id: 5",
        "    model_dir: /.../models/Qwen3.5-397B-A17B-exl3-5bpw",
        "  - id: 8",
        "    model_dir: /.../models/Qwen3.5-397B-A17B-exl3-8bpw",
        "  - id: 16",
    ...
    for label, key in fixed_8:
        lines.append(f"  # {label}")
        lines.append(f'  - key: "{key}"')
        lines.append("    source: 8")
        lines.append("")
...
def main():
    base_dir = Path("/home/ubuntu/path")
    ...
    print(f"PENALTY (superlinear, power=1.5): 2={PENALTY[2]:.4f}, 3={PENALTY[3]:.4f}, 4={PENALTY[4]:.4f}, 5={PENALTY[5]:.4f}, 8={PENALTY[8]:.4f}")

    targets = [2.2, 2.5, 2.8, 3.0, 3.3, 3.35, 3.37, 3.5, 3.6, 3.7, 3.8, 3.9, 4.0, 4.1, 4.5]
    ...
    for target in targets:
        ...
        n8, n5, n4, n3, n2 = split
        print(f"  Optimal split: {n8} @ 8bpw, {n5} @ 5bpw, {n4} @ 4bpw, {n3} @ 3bpw, {n2} @ 2bpw")
        print(f"  Total error (KLD-weighted): {cost:.4f}")

        # Show which projections got 8bpw and 2bpw
        if n8 > 0:
            print(f"\n  Projections assigned 8bpw:")
            for layer in sorted(assignments):
                for proj in ("gate", "up", "down"):
                    if assignments[layer][proj] == 8:
                        lk = kld.get(layer, {})
        ...
        yaml_path = base_dir / f"greedy_optimization_{target}bpw_superlinear.yaml"
        yaml_path.write_text(yaml_out)
        print(f"\nSaved: {yaml_path}")

        json_path = base_dir / f"greedy_optimization_{target}bpw_superlinear.json"
        json_data = {
            ...
            "total_error": cost,
            "split": {"n8": n8, "n5": n5, "n4": n4, "n3": n3, "n2": n2},
        }
